...
%% @doc Start the circuit breaker gen_server
-spec start_link() -> {ok, pid()} | {error, term()}.
start_link() ->
    gen_server:start_link({local, ?MODULE}, ?MODULE, [], []).

%% @doc Record circuit breaker state (initialize or update)
%%
...
%% @doc Record circuit breaker state with explicit configuration
%%
%% **IMPORTANT**: This function only updates configuration/state, it does NOT check
%% timeout or trigger state transitions (e.g., open → half_open).
%%
...
%% @doc Check if request should be allowed (circuit breaker check)
%%
...
%% @doc Record successful provider call
...
%% @doc Record failed provider call
...
%% @doc Get detailed circuit breaker status for diagnostics
...
%% @doc Check if circuit is half-open
...
%% @doc Force recovery: transition from open to closed (bypass half-open)
...
%% @doc Force recovery: transition from open to half-open (immediate, bypass timeout)
%% Use with caution - only for manual recovery scenarios
...
%% @doc Get recovery status: time until recovery, recovery progress, etc.
...
%% @doc Reset recovery state: clear failure/success counts, reset to closed
%% Use with caution - only for manual recovery scenarios
...
%% @doc Check if circuit is closed
...
%% @doc Get default circuit breaker configuration
get_default_config() ->
    #{
...
%% @doc Maybe transition circuit breaker state based on timeout
...
%% @doc Update circuit breaker state on success
...
%% @doc Update circuit breaker state on failure
...
%% @doc Maybe transition to open state based on thresholds
...
%% @doc Get recent average latency for latency-based circuit breaker trigger
...
%% @doc Calculate error rate using sliding window (CP2+)
...
%% @doc Clean old events from sliding window (remove events outside window)
...
%% @doc Add event to sliding window (with automatic cleanup)
