...
%% @doc Publish ExecAssignment to CAF
...
do_publish_assignment_checked(RequestMap, DecisionRec) ->
    ...
    case AssignmentEnabled of
        ...
        true ->
            ...
            case check_tenant_allowed(TenantId) of
                false ->
    ...
    Result = router_tracing:with_span(
        ...
        fun() ->
            try
                ...
                Json = jsx:encode(ExecAssignment),
...
publish_with_retries(Subject, Json, AssignmentId, RequestId, TenantId, RequestMap) ->
    MaxRetries = get_config(caf_max_retries, ?DEFAULT_MAX_RETRIES),
...
-dialyzer({nowarn_function, calculate_backoff/1}).  %% False positive: function is called in retry loop
calculate_backoff(Retry) ->
    BaseMs = get_config(caf_retry_base_ms, ?DEFAULT_RETRY_BASE_MS),
...
calculate_deadline(ExpectedLatencyMs) ->
    Multiplier = get_config(caf_deadline_multiplier, ?DEFAULT_DEADLINE_MULTIPLIER),
    MinMs = get_config(caf_deadline_min_ms, ?DEFAULT_DEADLINE_MIN_MS),
    MaxMs = get_config(caf_deadline_max_ms, ?DEFAULT_DEADLINE_MAX_MS),
    
    Calculated = max(MinMs, ExpectedLatencyMs * Multiplier),
    Capped = min(MaxMs, Calculated),
    
    %% Log if deadline exceeds expected latency significantly
    case Capped > ExpectedLatencyMs * 10 of
        true ->
            log_deadline_exceeded(ExpectedLatencyMs, Capped);
        false ->
            ok
    end,
...
format_error_term(Error) when is_list(Error) ->
...
log_deadline_exceeded(ExpectedLatencyMs, DeadlineMs) ->
    router_logger:warn(<<"Deadline significantly exceeds expected latency">>, #{
        <<"expected_latency_ms">> => ExpectedLatencyMs,
        <<"deadline_ms">> => DeadlineMs,
        <<"ratio">> => DeadlineMs / ExpectedLatencyMs
    }).
...
%% @doc Validate assignment request map
...
%% @doc Validate route decision record
...
%% @doc Get assignment status (for monitoring)
...
get_assignment_status(AssignmentId) when is_binary(AssignmentId) ->
    ...
    case router_correlation_context:get_by_assignment_id(AssignmentId) of
        {ok, Context} ->
            #{
                assignment_id => AssignmentId,
