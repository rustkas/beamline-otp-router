...
%% @doc Initialize idempotency ETS table
...
%% @doc Check if key is duplicate (already processed)
...
%% @doc Check if key is duplicate and mark as processed atomically
...
%% @doc Check if key is duplicate and mark as processed atomically (with additional data)
...
%% @doc Check if key is already processed, register if not (atomic operation)
%% Key: Idempotency key (binary)
%% TTL: Time-to-live in milliseconds
...
%% @doc Check if key is already processed, register if not (with trace context)
%% Key: Idempotency key (binary)
%% TTL: Time-to-live in milliseconds
%% AdditionalData: Optional additional data (map or undefined)
...
%% @doc Mark operation as completed and save result snapshot
%% Key: Idempotency key (binary)
...
%% @doc Mark operation as completed and save result snapshot (with error code)
%% Key: Idempotency key (binary)
%% Status: completed | failed
%% ResultSnapshot: Optional result data to store (map or undefined)
...
%% @doc Extract idempotency key from headers or payload
...
%% @doc Remember key with TTL (mark as processed)
%% TTL: Time-to-live in milliseconds
...
%% @doc Evict specific key (manual cleanup)
evict(Key) ->
...
%% @doc Evict all expired entries (mass cleanup)
...
%% @doc Evict all entries (emergency cleanup)
...
%% @doc Reset all idempotency state (for testing)
...
%% @doc Periodic cleanup (called by timer)
...
%% @doc Extract trace_id from TraceContext or AdditionalData
...
%% @doc Extract span_id from TraceContext or AdditionalData
...
%% @doc Extract request_hash from AdditionalData
...
%% @doc Start periodic cleanup timer
