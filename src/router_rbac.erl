...
-compile({nowarn_unused_function, [
    check_role_permission/5,        % Used: line 587 (called within try-catch)
    check_conditional_permissions/4  % Used: line 597 (called by check_role_permission/5)
]}).
...
%% @doc Internal initialization function (called from init/1 and reset_all)
...
%% @doc Check if user can access resource with action
...
%% @doc Check if user can access resource with action (simplified, no context)
...
%% @doc Get all roles for user in tenant
...
%% @doc Check if user has specific permission
...
%% @dialyzer {nowarn_function, check_role_permission/5}
-dialyzer({nowarn_function, check_role_permission/5}).
check_role_permission(RoleId, Action, Resource, Context, State) ->
    try
        RolesTable = State#state.roles_table,
        ...
        case ets:lookup(RolesTable, RoleId) of
            [{RoleId, Permissions}] ->
                ...
                PermissionKey = {Action, Resource},
                lists:member(PermissionKey, Permissions) orelse
                %% Check conditional permissions
                check_conditional_permissions(Permissions, Action, Resource, Context);
            [] ->
                false
        end
    catch
        error:{badarg, _} ->
            %% Table not accessible - deny access
            false;
        _:_ ->
...
%% Check conditional permissions from a list of permissions
%% @dialyzer {nowarn_function, check_conditional_permissions/4}
-dialyzer({nowarn_function, check_conditional_permissions/4}).
check_conditional_permissions(_Permissions, _Action, _Resource, _Context) ->
    %% For now, conditional permissions are handled at user level
    %% This function can be extended for role-level conditional permissions
    false.
...
%% @doc Validate user input (UserId and TenantId)
