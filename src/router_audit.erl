...
%% @doc Get audit retention days from configuration
...
%% @doc Log policy action
-spec log_policy_action(binary(), binary(), binary(), binary(), map()) -> ok.
log_policy_action(TenantId, UserId, Action, PolicyId, Details) ->
    Entry = #audit_entry{
...
%% @doc Get audit entries for tenant (with pagination)
...
get_audit_entries(TenantId, Limit, Offset) ->
    try
        ...
        %% Match all entries for tenant
        Pattern = {'$1', #audit_entry{tenant_id = TenantId, _ = '_'}},
        Matches = ets:match(?AUDIT_TABLE, Pattern),
        
        %% Sort by timestamp (descending) and paginate
        Entries = [Entry || [Entry] <- Matches],
...
%% @doc Get client IP from process dictionary or context
...
%% @doc Clear all audit entries (for testing)
%% @returns ok
...
%% @doc Enforce size limit by evicting oldest entries if needed
...
evict_expired_entries(RetentionDays) ->
    ...
    MatchSpec = [
        {
            {'$1', #audit_entry{ts = '$2', _ = '_'}},
            [{'<', '$2', CutoffTime}],
            ['$1']
        }
    ],
    Keys = ets:select(?AUDIT_TABLE, MatchSpec),
