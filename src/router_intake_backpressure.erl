...
%% 
%% Reference: docs/dev/CP2_TECH_DEBT_SUMMARY.md (Router: Backpressure Logic section)
...
%% @doc Check backpressure status for a subject
...
%% @doc Get backpressure thresholds with validation
...
%% @doc Get current backpressure status (without triggering checks)
...
%% @doc Track backpressure status in history
...
%% @doc Internal backpressure status calculation
...
%% @doc Get JetStream pending messages for a subject
%% 
...
%% @doc Try to query JetStream consumer info via NATS API (real-time)
%% 
...
%% @doc Prepare JetStream query configuration from subject
...
%% @doc Extract consumer name from subject
...
%% @doc Build JetStream API query subject
...
%% @doc Get JetStream pending from cached ETS table (fallback)
...
%% @doc Update pending cache with new value
...
%% @doc Get processing latency P95 for a subject
%% 
%% ⚠️ STUB IMPLEMENTATION ⚠️
...
%% @doc Try to calculate P95 from histogram metrics (real-time)
%% 
...
%% @doc Convert error to binary for metrics
...
%% @doc Get latency samples for subject from samples table
...
%% @doc Calculate percentile from sorted list of values
...
%% @doc Try to calculate P95 from Prometheus histogram (if Prometheus integration available)
%% 
%% Returns: {ok, P95Ms} | {error, Reason}
%%
...
%% @doc Get processing latency P95 from cached ETS table (fallback)
...
%% @doc Update latency cache with calculated P95 value
...
%% @doc Get in-flight message count for a subject
...
%% @doc Emit backpressure status metric
...
%% @doc Emit backpressure triggered metric
...
%% @doc Get detailed backpressure status with all metrics
...
%% @doc Get backpressure policy for a subject
...
%% @doc Get default backpressure policy
...
%% @doc Validate backpressure policy
...
%% @doc Apply backpressure policy to determine action
...
%% @doc Get backpressure events for a subject
...
%% @doc Get aggregated backpressure metrics for a subject
...
%% @doc Get backpressure recovery status
...
%% @doc Check if backpressure has recovered
...
%% @doc Get previous backpressure status
...
%% @doc Track backpressure recovery
