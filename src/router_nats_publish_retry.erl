...
%% @doc Publish with retry using default configuration
...
%% @doc Publish with retry using explicit configuration
-spec publish_with_retry(binary(), binary(), fun((binary(), binary()) -> ok | {error, term()}), 
...
%% @doc Calculate backoff delay with jitter
...
%% @doc Get default retry configuration
...
%% @doc Classify error type for logging (internal helper)
