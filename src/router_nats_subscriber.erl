...
init([]) ->
    ...
    case router_nats:subscribe(?SUBJECT, fun handle_nats_message/2, Timeout) of
        ok ->
            {ok, #state{
...
handle_info({nats_message, Subject, Payload}, State) ->
    handle_nats_message(Subject, Payload),
    {noreply, State};
%% New format with ReplyTo (ignored for this subscriber)
handle_info({nats_message, Subject, Payload, _ReplyTo, _Headers, _MsgId}, State) ->
    handle_nats_message(Subject, Payload),
    {noreply, State};
...
-spec handle_nats_message(binary(), binary()) -> ok.
handle_nats_message(Subject, Payload) ->
    ...
    case PayloadSize > MaxPayloadSize of
        true ->
            ...
            send_error_response(Subject, #{}, <<"invalid_request">>, <<"Payload size exceeds limit">>),
            ok;
        false ->
    ...
    case jsx:decode(Payload, [return_maps]) of
        Request when is_map(Request) ->
                    ...
                    case Version of
                        <<"1">> ->
                            handle_decide_request(Subject, Request);
                        undefined ->
                            ...
                            send_error_response(Subject, Request, <<"invalid_request">>, <<"Missing version field">>);
                        _ ->
                            %% Build log context with CP1 correlation fields (if available)
                            LogContext = build_log_context(Request, #{
                                <<"version">> => Version,
                            ...
                            send_error_response(Subject, Request, <<"invalid_request">>, <<"Unsupported version">>)
                    end;
                Error ->
                    ...
                    send_error_response(Subject, #{}, <<"invalid_request">>, <<"Failed to parse JSON">>)
            end
    end.

%% Internal: Handle DecideRequest
-spec handle_decide_request(binary(), map()) -> ok.
handle_decide_request(Subject, Request) ->
...
-spec send_error_response(binary(), map(), binary(), binary()) -> ok.
send_error_response(Subject, Request, ErrorCode, ErrorMessage) ->
