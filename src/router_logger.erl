...
%% @doc Check if a log level should be logged
...
%% @doc Internal: Emit structured JSON log
...
%% @doc Build structured log entry
...
%% @doc Helper function to add CP1 field if present
...
%% @doc Extract field from context (supports both binary and atom keys)
...
%% @doc Filter PII from context map
...
%% @doc Convert log level to binary
...
%% @doc Get date string (YYYY-MM-DD)
