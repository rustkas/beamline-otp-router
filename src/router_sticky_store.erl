...
%% @doc Start sticky store
start_link() ->
    gen_server:start_link({local, ?MODULE}, ?MODULE, [], []).

%% @doc Initialize sticky store
init([]) ->
    Table = ets:new(?TABLE, [
        set,
        named_table,
...
%% @doc Handle get_provider request
