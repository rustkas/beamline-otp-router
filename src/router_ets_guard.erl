...
%% @doc Verify table invariants against expected specification
...
%% @doc Verify table invariants (returns violations list)
...
%% @doc Log violations (only in production, not in tests)
log_violations(Table, Violations) ->
...
%% @doc Check specification keys (fail-fast validation)
...
%% @doc Validate specification (fail-fast on invalid keys)
...
%% @doc Get table name for logging/metrics
