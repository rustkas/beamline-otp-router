...
-export([decide/3, execute_post_processors/3]).
...
%% @doc Main decision function - with extensions pipeline support
...
%% @doc Execute post-processor extensions
-spec execute_post_processors(list(), map(), map()) -> {ok, map(), map()} | {error, {atom(), map()}}.
...
%% @doc Calculate pipeline complexity metrics
...
%% @doc Get pipeline complexity for a policy
...
check_sticky(undefined, _Context) ->
    {error, not_found};
check_sticky(Sticky, _Context) when not(is_map(Sticky)) ->
    {error, not_found};
check_sticky(Sticky, Context) ->
    case maps:get(<<"enabled">>, Sticky, false) of
        true ->
            ...
            case TenantId =/= undefined andalso SessionValue =/= undefined of
                true ->
...
-spec check_fallback(map() | undefined) -> {ok, binary()} | {error, no_fallback}.
...
store_sticky_provider(TenantId, Sticky, ProcessedContext, ProviderId) ->
    case TenantId =/= undefined andalso is_map(Sticky) andalso maps:get(<<"enabled">>, Sticky, false) of
        true ->
            SessionKeyName = maps:get(<<"session_key">>, Sticky, <<"session_id">>),
            SessionValue = maps:get(SessionKeyName, ProcessedContext, undefined),
            case SessionValue =/= undefined of
                true ->
...
-spec log_pipeline_complexity(map(), binary(), binary() | undefined) -> ok.
log_pipeline_complexity(Complexity, PolicyId, TenantId) ->
...
%% @doc Validate provider selection configuration
...
%% @doc Get provider selection status for a policy
...
%% @doc List available providers for a policy
...
-spec validate_sticky(map() | undefined) -> ok | {error, term()}.
...
-spec validate_fallback(binary() | undefined, list()) -> ok | {error, term()}.
...
-spec has_available_providers(map(), binary() | undefined, list()) -> boolean().
...
-spec get_providers_from_fallback(binary() | undefined, list()) -> [binary()].
