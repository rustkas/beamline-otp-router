...
%% @doc Spawn clients using strategy from config (default: concurrent)
...
%% @doc Spawn clients with explicit strategy
...
spawn_clients(NumClients, RequestsPerClient, Strategy) ->
    case Strategy of
        sync -> spawn_clients_sync(NumClients, RequestsPerClient);
        concurrent -> spawn_clients_concurrent(NumClients, RequestsPerClient)
    end.

%% @doc Spawn clients synchronously (simple loop, CI-friendly)
%% Each client runs sequentially, one after another
...
%% @doc Spawn clients concurrently (N processes, load-friendly)
...
%% @doc Get R10 profile from CT config or environment
