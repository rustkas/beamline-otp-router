%% @doc Multi-Instance Network Partition Tests
%% 
%% Tests for split-brain and distributed partition scenarios.
%% Runs only with ROUTER_TEST_LEVEL=heavy.
%%
%% @test_category network_partition, heavy, slow
-module(router_network_partition_multi_SUITE).
...
-export([
    ...
    test_leader_election_after_healing/1,
    test_split_brain_recovery/1
]).
...
all() ->
    case os:getenv("ROUTER_TEST_LEVEL") of
        "heavy" -> [{group, multi_instance_tests}];
        _ -> []
    end.
...
groups() ->
    [{multi_instance_tests, [sequence], [
        ...
        test_leader_election_after_healing,
...
test_leader_election_after_healing(_Config) ->
    {ok, _} = application:ensure_all_started(beamline_router),
    InitialMetrics = router_network_partition_helper:get_metrics_snapshot(),
...
test_split_brain_recovery(_Config) ->
    {ok, _} = application:ensure_all_started(beamline_router),
    InitialMetrics = router_network_partition_helper:get_metrics_snapshot(),
