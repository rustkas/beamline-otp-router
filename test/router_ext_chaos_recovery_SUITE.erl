...
test_mass_degradation(_Config) ->
    ...
    MassCounter = router_test_init:ensure_ets_table(mass_counter, [named_table, set, private]),
    meck:expect(router_nats, request, fun(_Subject, _Payload, _Timeout) ->
        RequestNum = case ets:lookup(MassCounter, count) of
            [] -> 1;
            [{count, C}] -> C + 1
        end,
...
test_recovery_after_failure(_Config) ->
    ...
    RecoveryCounter = router_test_init:ensure_ets_table(recovery_counter, [named_table, set, private]),
    meck:expect(router_nats, request, fun(_Subject, _Payload, _Timeout) ->
