...
init_per_suite(Config) ->
    ...
    _ = router_test_init:ensure_ets_table(timeout_counter, [set, public]),
    _ = router_test_init:ensure_ets_table(slow_counter, [set, public]),
    _ = router_test_init:ensure_ets_table(failure_counter, [set, public]),
...
init_per_testcase(_TC, Config) ->
    ...
    TimeoutCounter = router_test_init:ensure_ets_table(timeout_counter, [set, public]),
    SlowCounter = router_test_init:ensure_ets_table(slow_counter, [set, public]),
    FailureCounter = router_test_init:ensure_ets_table(failure_counter, [set, public]),
...
test_with_timeouts(_Config) ->
    ...
    Counter = router_test_init:ensure_ets_table(timeout_counter, [set, public]),
    ets:delete_all_objects(Counter),
    meck:expect(router_nats, request, fun(_Subject, _Payload, _Timeout) ->
        N = case ets:lookup(Counter, count) of [] -> 1; [{count, C}] -> C + 1 end,
...
test_with_degraded_instance(_Config) ->
    ...
    Counter = router_test_init:ensure_ets_table(slow_counter, [set, public]),
    ets:delete_all_objects(Counter),
    meck:expect(router_nats, request, fun(_Subject, _Payload, _Timeout) ->
        N = case ets:lookup(Counter, count) of [] -> 1; [{count, C}] -> C + 1 end,
        ets:insert(Counter, {count, N}),
...
test_circuit_breaker_activation(_Config) ->
    ...
    Counter = router_test_init:ensure_ets_table(failure_counter, [set, public]),
    ets:delete_all_objects(Counter),
    meck:expect(router_nats, request, fun(_Subject, _Payload, _Timeout) ->
        N = case ets:lookup(Counter, count) of [] -> 1; [{count, C}] -> C + 1 end,
        ets:insert(Counter, {count, N}),
