...
test_redelivery_until_ack_or_maxdeliver(_Config) ->
    ...
    %% Track NAK calls
    NakCalls = router_test_init:ensure_ets_table(nak_calls, [named_table, set, public]),
    ets:insert(NakCalls, {count, 0}),
    
    meck:expect(router_nats, nak_message, fun(MsgId) ->
        [{count, Count}] = ets:lookup(NakCalls, count),
    ...
    [{count, FinalCount}] = ets:lookup(NakCalls, count),
...
test_ack_latency_within_target(_Config) ->
    ...
    AckTiming = router_test_init:ensure_ets_table(ack_timing, [named_table, set, public]),
    ...
    %% Verify latency
    [{<<"msg-latency-test">>, Latency}] = ets:lookup(AckTiming, <<"msg-latency-test">>),
