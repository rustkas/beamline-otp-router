...
test_extension_flapping(_Config) ->
    ...
    FlapCounter = router_test_init:ensure_ets_table(flap_counter, [named_table, set, private]),
    meck:expect(router_nats, request, fun(_Subject, _Payload, _Timeout) ->
...
test_latency_degradation(_Config) ->
    ...
    LatencyCounter = router_test_init:ensure_ets_table(latency_counter, [named_table, set, private]),
    meck:expect(router_nats, request, fun(_Subject, _Payload, _Timeout) ->
