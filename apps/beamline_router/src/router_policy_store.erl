...
-record(state, {
    table :: ets:table(),
    index_table :: ets:table() | undefined,  %% Secondary index: tenant_id -> [policy_id]
...
handle_call({upsert_policy, TenantId, Policy, CorrelationId}, _From, State) ->
    ...
    PolicyId = element(#policy.policy_id, Policy),
    Meta = build_meta(#{tenant_id => TenantId, table => ?TABLE, policy_id => PolicyId}, CorrelationId),
...
load_from_fixture(TenantId, PolicyId) ->
    ...
    case file:read_file(FixturePath) of
        {ok, JsonData} ->
            case jsx:decode(JsonData, [{return_maps, true}]) of
...
rebuild_index(Table, IndexTable) ->
    ...
    exec_with_telemetry(rebuild_index, Meta, fun() ->
        ...
        try
            ...
            RebuildResult = rebuild_index_cont(
                Table,
                IndexTable,
                ets:select(Table, [{Pattern, Guard, Body}], 100)
            ),
...
do_list_policies(TenantId, Table, IndexTable) ->
    ...
    try
        ...
        SortedResult = lists:sort(
            fun(P1, P2) -> element(#policy.policy_id, P1) < element(#policy.policy_id, P2) end,
            ValidPolicies
        ),
...
policy_to_map(Policy) ->
    #{
        ...
        ~"defaults" => Policy#policy.defaults,
        ~"escalate_on" => Policy#policy.escalate_on,
        ~"weights" => Policy#policy.weights,
        ~"fallback" => Policy#policy.fallback,
        ~"fallbacks" => Policy#policy.fallbacks,
        ~"sticky" => Policy#policy.sticky,
        ~"rate_limit" => Policy#policy.rate_limit,
        ~"circuit_breaker" => Policy#policy.circuit_breaker,
        ~"pre" => Policy#policy.pre,
        ~"validators" => Policy#policy.validators,
        ~"post" => Policy#policy.post,
        ~"allow_rules" => Policy#policy.allow_rules,
        ~"metadata" => Policy#policy.metadata
    }.
...
-spec check_size_limit() -> {ok, integer()} | {error, exceeded, integer(), integer()} | {error, no_limit_configured} | {error, invalid_limit}.
check_size_limit() ->
