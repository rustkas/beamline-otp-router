...
check_nats_connection() ->
    try
        %% In test mode, skip NATS check (mocked NATS may not match production format)
        case os:getenv("ROUTER_DEPLOYMENT_TEST_MODE") of
            "1" ->
                #{passed => true, status => test_mode_skip};
            _ ->
...
check_memory() ->
    try
        ...
        MinMemory =
            case os:getenv("ROUTER_DEPLOYMENT_TEST_MODE") of
                "1" ->
                    64 * 1024 * 1024;  %% 64MB for tests
                _ ->
                    128 * 1024 * 1024     %% 128MB for prod
            end,
...
get_previous_deployment() ->
    ...
    case ets:last(deployment_history) of
        '$end_of_table' ->
            {error, no_previous_version};
        LastKey ->
            case ets:prev(deployment_history, LastKey) of
                '$end_of_table' ->
                    {error, no_previous_version};
                PrevKey ->
