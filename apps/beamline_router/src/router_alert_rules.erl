...
-export([
    ...
    validate_rule/1,
    add_rule/1,  %% For dynamic rule addition
    delete_dynamic_rules/0
]).
...
get_all_rules() ->
    StaticRules = get_r13_fault_rules() ++
                  get_circuit_breaker_rules() ++
                  get_error_rate_rules() ++
                  get_performance_rules(),
    DynamicRules = case ets:whereis(router_dynamic_alert_rules) of
        undefined -> [];
        _ ->
            ets:foldl(fun({_Id, Rule}, Acc) -> [Rule | Acc] end, [], router_dynamic_alert_rules)
    end,
...
get_rule(RuleId) ->
    ...
    %% Search for rule with matching id field
    case lists:search(fun(Rule) -> 
        maps:get(id, Rule, undefined) =:= RuleId 
    end, AllRules) of
        {value, Rule} ->
            {ok, Rule};
        false ->
...
-spec add_rule(#alert_rule{} | map()) -> ok | {error, term()}.
add_rule(Rule) when is_record(Rule, alert_rule) ->
    ...
    RuleMap = #{
        ...
        window_ms => Rule#alert_rule.window_ms,
        ...
        enabled => true,
        name => Rule#alert_rule.id,
        description => <<"Dynamically added rule">>,
        labels => #{}
    },
    add_rule(RuleMap);
add_rule(Rule) when is_map(Rule) ->
    %% Ensure ETS table exists for dynamic rules
    case ets:whereis(router_dynamic_alert_rules) of
        undefined ->
            ets:new(router_dynamic_alert_rules, [set, public, named_table]);
        _ ->
            ok
    end,
    RuleId = maps:get(id, Rule),
    ets:insert(router_dynamic_alert_rules, {RuleId, Rule}),
    ok.

-spec delete_dynamic_rules() -> ok.
delete_dynamic_rules() ->
    case ets:whereis(router_dynamic_alert_rules) of
        undefined -> ok;
        _ ->
            ets:delete_all_objects(router_dynamic_alert_rules),
            ok
    end.
