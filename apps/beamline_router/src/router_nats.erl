...
-record(state, {
    ...
    %% List of NATS core subscriptions to resubscribe after reconnect
    subscriptions = [] :: list()
}).

-record(nats_subscription, {
    subject :: binary(),
    owner_pid :: pid(),
    callback :: fun((binary(), binary()) -> ok) | undefined,
...
handle_call({subscribe, Subject, Callback, Timeout}, {OwnerPid, _Tag}, State) ->
    case do_subscribe(Subject, Callback, Timeout, OwnerPid, State) of
        {ok, NewState} ->
            {reply, ok, NewState};
        {error, Reason} ->
            {reply, {error, Reason}, State}
    end;
handle_call({request, Subject, Payload, Timeout}, _From, State) ->
    Result = do_request(Subject, Payload, Timeout, State),
    {reply, Result, State};
...
subscribe(Subject, Callback, Timeout) ->
    gen_server:call(?MODULE, {subscribe, Subject, Callback, Timeout}).

request(Subject, Payload, Timeout) ->
    gen_server:call(?MODULE, {request, Subject, Payload, Timeout}).

%% ============================================================================
%% Internal Functions
%% ============================================================================

-spec do_subscribe(binary(), fun((binary(), binary()) -> ok) | undefined, non_neg_integer(), pid(), #state{}) ->
    {ok, #state{}} | {error, term()}.
do_subscribe(Subject, Callback, Timeout, OwnerPid, State) ->
    case State#state.connection_state of
        ?CONN_STATE_CONNECTED ->
            do_subscribe_internal(Subject, Callback, Timeout, OwnerPid, State);
        _ ->
            {ok, upsert_subscription(Subject, Callback, Timeout, OwnerPid, State)}
    end.

-spec do_subscribe_internal(binary(), fun((binary(), binary()) -> ok) | undefined, non_neg_integer(), pid(), #state{}) ->
    {ok, #state{}} | {error, term()}.
do_subscribe_internal(Subject, Callback, Timeout, OwnerPid, State) ->
    ConnectionPid = State#state.connection_pid,
    case is_pid(ConnectionPid) andalso is_process_alive(ConnectionPid) of
        true ->
            case is_stub_connection(ConnectionPid) of
                true ->
                    ConnectionPid ! {subscribe, Subject, #{}},
                    {ok, upsert_subscription(Subject, Callback, Timeout, OwnerPid, State)};
                false ->
                    NotifyFun = fun(_SRef, MsgSubject, MsgPayload, MsgOpts) ->
                        ReplyTo = maps:get(reply_to, MsgOpts, undefined),
                        HeaderVal = maps:get(header, MsgOpts, #{}),
                        Headers =
                            case HeaderVal of
                                H when is_map(H) -> H;
                                _ -> #{}
                            end,
                        OwnerPid ! {nats_message, MsgSubject, MsgPayload, ReplyTo, Headers, undefined},
                        maybe_invoke_callback(Callback, MsgSubject, MsgPayload),
                        ok
                    end,
                    case nats:sub(ConnectionPid, Subject, #{notify => NotifyFun}) of
                        {ok, _SubRef} ->
                            {ok, upsert_subscription(Subject, Callback, Timeout, OwnerPid, State)};
                        {error, Reason} ->
                            {error, Reason}
...
-spec do_request(binary(), binary(), non_neg_integer(), #state{}) -> {ok, binary()} | {error, term()}.
do_request(Subject, Payload, Timeout, State) ->
    case State#state.connection_state of
        ?CONN_STATE_CONNECTED ->
            do_request_internal(Subject, Payload, Timeout, State#state.connection_pid);
        _ ->
            {error, not_connected}
    end.

-spec do_request_internal(binary(), binary(), non_neg_integer(), pid() | undefined) ->
    {ok, binary()} | {error, term()}.
do_request_internal(_Subject, _Payload, _Timeout, undefined) ->
    {error, invalid_connection};
do_request_internal(Subject, Payload, Timeout, ConnectionPid) ->
    case is_pid(ConnectionPid) andalso is_process_alive(ConnectionPid) of
        true ->
            case is_stub_connection(ConnectionPid) of
                true ->
                    {ok, <<>>};
                false ->
                    case nats:request(ConnectionPid, Subject, Payload, #{timeout => Timeout}) of
                        {ok, {ReplyPayload, _Opts}} -> {ok, ReplyPayload};
...
maybe_invoke_callback(undefined, _Subject, _Payload) ->
    ok;
maybe_invoke_callback(Callback, Subject, Payload) when is_function(Callback, 2) ->
    _ = catch Callback(Subject, Payload),
    ok;
maybe_invoke_callback(_Callback, _Subject, _Payload) ->
    ok.

upsert_subscription(Subject, Callback, Timeout, OwnerPid, State) ->
    Sub = #nats_subscription{
        subject = Subject,
        owner_pid = OwnerPid,
        callback = Callback,
        timeout_ms = Timeout
    ...
    Filtered = lists:filter(
        fun(#nats_subscription{subject = S, owner_pid = P}) ->
            not (S =:= Subject andalso P =:= OwnerPid)
        end,
        Subscriptions
    ),
    State#state{subscriptions = [Sub | Filtered]}.

is_stub_connection(ConnectionPid) ->
    case process_info(ConnectionPid, dictionary) of
        {dictionary, Dict} ->
            case lists:keyfind(stub_connection, 1, Dict) of
                {stub_connection, true} -> true;
                _ -> false
            end;
        _ ->
            false
...
connect_to_nats(Url, Config) ->
    ...
    FinalOptions = case maps:get(enabled, TLSConfig, false) of
        true ->
            HostVal = maps:get(host, Url, "localhost"),
            HostStr = if is_binary(HostVal) -> binary_to_list(HostVal); true -> HostVal end,
            SSLOptions = [
                {certfile, maps:get(cert_file, TLSConfig, "")},
                {keyfile, maps:get(key_file, TLSConfig, "")},
                {cacertfile, maps:get(ca_cert_file, TLSConfig, "")},
                {verify, case maps:get(verify, TLSConfig, true) of true -> verify_peer; false -> verify_none end},
    ...
    %% Note: using connect/3 to match library API
...
handle_connection_restored(Pid, State) ->
    ...
    RetryState2 = resubscribe_jetstream_consumers(RetryState),
    resubscribe_subscriptions(RetryState2).
...
resubscribe_subscriptions(State) ->
    lists:foldl(
        fun(#nats_subscription{subject = Subject, callback = Callback, timeout_ms = Timeout, owner_pid = OwnerPid},
            AccState) ->
            case do_subscribe_internal(Subject, Callback, Timeout, OwnerPid, AccState) of
                {ok, NewState} ->
                    router_logger:info(~"NATS subscription resubscribed after reconnection", #{
                        ~"subject" => Subject
                    }),
                    NewState;
                {error, Reason} ->
                    router_logger:warn(~"NATS subscription resubscription failed", #{
                        ~"subject" => Subject,
                        ~"reason" => sanitize_error(Reason)
                    }),
                    AccState
