...
%% Wrapper around router_logger:sanitize_error_for_logging/1
sanitize_error_for_logging(Error) ->
    router_logger:sanitize_error_for_logging(Error).
...
handle(#{id := Id, subject := Subject} = Msg, Ctx) ->
  ...
  case DeliveryCount >= Max of
    true ->
      ...
      AssignmentId =
        case Ctx of
          #{assignment_id := ProvidedAssignmentId} -> ProvidedAssignmentId;
          _ -> extract_assignment_id(Subject)
        end,
...
clear_delivery_count(Id) ->
  try
    ...
    error:badarg ->
      %% Table might not exist in tests or shut down
      ok
  end,
  ok.
