...
process_test_message() ->
    ...
    ResultJson = case jsx:encode(Result) of
        {incomplete, _} -> error(unexpected_incomplete_json);
        Json -> Json
    end,
    MsgId = ~"msg-test",
    Payload = encode_json(ResultJson),
    State = #state{
        connection = undefined,
        result_subject = ~"caf.exec.result.v1",
        usage_subject = ~"beamline.usage.v1.metered",
        js_durable_group = ~"router-results",
        correlation_map = #{}
    },
    router_result_consumer:handle_info({nats_message, ~"caf.exec.result.v1", Payload, #{}, MsgId}, State),
    ok.
...
%% Convert jsx output to a binary payload.
-spec encode_json(jsx:json_text()) -> binary().
encode_json(JsonText) ->
    iolist_to_binary(JsonText).
...
-spec simulate_router_restart() -> ok.
