    ...
    Metadata = case is_map(Ctx) of
        ...
        false ->
            ...
            case erlang:function_exported(grpcbox_ctx, metadata, 1) of
                true ->
                    try apply(grpcbox_ctx, metadata, [Ctx]) of
                        undefined -> [];
                        M when is_list(M) -> M;
                        _ -> []
                    catch
...
%% Keys must be provided via environment variable or secure config, except in test override mode
-spec get_admin_key() -> binary().
get_admin_key() ->
    case test_admin_key_override() of
        {ok, Key} ->
            Key;
        error ->
            fetch_admin_key_from_config()
    end.

%% Internal: Fetch API key from production configuration (app env / os env)
fetch_admin_key_from_config() ->
...
%% Internal: Override hook for tests that need a deterministic admin API key
test_admin_key_override() ->
    case os:getenv("ROUTER_ADMIN_API_TEST_OVERRIDE") of
        false ->
            error;
        undefined ->
            error;
        Key when is_list(Key) ->
            {ok, normalize_test_admin_key(list_to_binary(Key))};
        Key when is_binary(Key) ->
            {ok, normalize_test_admin_key(Key)};
        _ ->
            {ok, test_admin_key_constant()}
    end.

normalize_test_admin_key(Key) when is_binary(Key) ->
    case is_valid_non_empty_key(Key) of
        true -> Key;
        false -> test_admin_key_constant()
    end.

test_admin_key_constant() ->
    ~"test-admin-key".
...
decode_get_request(Request) when is_binary(Request) ->
    try
        ...
        if
            TenantId =/= undefined, PolicyId =/= undefined ->
                {ok, TenantId, PolicyId};
            true ->
                {error, invalid_request}
        end
...
get_user_id(Ctx) ->
    ...
    case erlang:function_exported(grpcbox_metadata, get, 2) of
        true ->
            try apply(grpcbox_metadata, get, [Ctx, ~"x-user-id"]) of
                undefined -> extract_user_id_from_map(Ctx);
                UserId -> UserId
            catch
                _:_ -> extract_user_id_from_map(Ctx)
            end;
        false ->
            extract_user_id_from_map(Ctx)
    end.
