...
-define(DEFAULT_REQUEST_COUNT_THRESHOLD, 50).   %% Request count threshold for targeted tenant abuse
...
-spec check_abuse_pattern(binary(), atom()) -> {ok, normal} | {abuse, heavy_payload, map()} | {abuse, targeted_tenant, map()}.
    ...
    try
        ...
        case ets:lookup(?PAYLOAD_TRACKING_TABLE, Key) of
            [] ->
                {ok, normal};
            [{Key, Stats}] ->
                %% 1. Check for targeted tenant flood (too many requests)
                RequestCountThreshold = get_request_count_threshold(),
                case Stats#payload_stats.total_requests >= RequestCountThreshold of
                    true ->
                        AbuseContext = #{
                            ~"tenant_id" => TenantId,
                            ~"endpoint" => atom_to_binary(Endpoint, utf8),
                            ~"total_requests" => Stats#payload_stats.total_requests,
                            ~"reason" => ~"request_count_threshold_exceeded"
                        },
                        {abuse, targeted_tenant, AbuseContext};
                    false ->
                        %% 2. Check for heavy payload pattern (high ratio of large payloads)
...
%% Internal: Get request count threshold
get_request_count_threshold() ->
    application:get_env(beamline_router, abuse_request_count_threshold, ?DEFAULT_REQUEST_COUNT_THRESHOLD).
