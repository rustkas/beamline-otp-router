...
-export([handle_decide_message/4, handle_decide_message/5]). %% Exported for testing
...
init([]) ->
    %% Get configuration
    DecideSubject0 = get_config(decide_subject, ?DEFAULT_DECIDE_SUBJECT),
    JSDurableGroup0 = get_config(nats_js_durable_group_decide, ?DEFAULT_JS_DURABLE_GROUP),
    DecideSubject = normalize_binary_config(DecideSubject0, ?DEFAULT_DECIDE_SUBJECT),
    JSDurableGroup = normalize_binary_config(JSDurableGroup0, ?DEFAULT_JS_DURABLE_GROUP),
...
handle_info({nats_message, Subject, Payload}, State) ->
    ...
    handle_decide_message(Subject, Payload, #{}, undefined, undefined),
    {noreply, State};
handle_info({nats_message, Subject, Payload, Headers}, State) ->
    %% Backward compatibility: headers but no msg_id
    handle_decide_message(Subject, Payload, Headers, undefined, undefined),
    {noreply, State};
handle_info({nats_message, Subject, Payload, Headers, MsgId}, State) ->
    ...
    handle_decide_message(Subject, Payload, Headers, MsgId, undefined),
    {noreply, State};
%% New format with ReplyTo
handle_info({nats_message, Subject, Payload, ReplyTo, Headers, MsgId}, State) ->
    %% Track delivery count for MaxDeliver exhaustion detection
    track_delivery_count(MsgId),
    handle_decide_message(Subject, Payload, Headers, MsgId, ReplyTo),
    {noreply, State};
...
-spec handle_decide_message(binary(), binary(), map(), binary() | undefined, binary() | undefined) -> ok.
handle_decide_message(Subject, Payload, Headers, MsgId, ReplyTo) ->
    handle_decide_message_internal(Subject, Payload, Headers, MsgId, ReplyTo).

-spec handle_decide_message_internal(binary(), binary(), map(), binary() | undefined, binary() | undefined) -> ok.
handle_decide_message_internal(Subject, Payload, Headers, MsgId, ReplyTo) ->
    try
        ...
        case router_intake_backpressure:check_backpressure(Subject) of
            ...
            {backpressure_warning, _} ->
                ...
                process_message_with_backpressure_warning(Subject, Payload, Headers, MsgId, ReplyTo);
            {backpressure_inactive, _} ->
                %% No backpressure - process normally
                process_message_normal(Subject, Payload, Headers, MsgId, ReplyTo)
        end
    catch
        Class:Reason:Stack ->
            %% Error handling
...
-spec process_message_normal(binary(), binary(), map(), binary() | undefined, binary() | undefined) -> ok.
process_message_normal(Subject, Payload, Headers, MsgId, ReplyTo) ->
    try
        ...
        case PayloadSize > MaxPayloadSize of
            true ->
                ...
                %% Emit abuse metric for heavy payload
                TenantId = extract_tenant_id_from_payload(Payload),
                case TenantId of
                    undefined -> ok;
                    TId -> emit_abuse_metric(heavy_payload, TId, Context)
                end,
                ok;
            false ->
                ...
                validate_and_process_message(Subject, Payload, Headers, MsgId, ReplyTo)
        end
    catch
        Class:Reason:Stack ->
            ...
            %% Safely format stack trace
            StackStr = iolist_to_binary(io_lib:format("~p", [Stack])),
            CatchContext = #{
                ~"subject" => Subject,
                ~"msg_id" => MsgId,
                ~"reply_to" => ReplyTo,
...
check_and_track_payload(Subject, Payload, PayloadSize, Headers, MsgId) ->
    ...
    case TenantId of
        ...
        TId when is_binary(TId) ->
            ...
            %% Track per-tenant payload
            router_payload_tracker:track_payload(TId, decide, PayloadSize),
            %% Track aggregate payload for flood detection
            router_payload_tracker:track_payload(<<"_aggregate_">>, decide, PayloadSize),
...
handle_abuse_patterns(TId, Subject, Payload, Headers, MsgId) ->
    ...
    case router_payload_tracker:check_abuse_pattern(TId, decide) of
        ...
        {abuse, targeted_tenant, AbuseContext} ->
            log_abuse_event(targeted_tenant, TId, Subject, Payload, Headers, MsgId, AbuseContext),
            emit_abuse_metric(targeted_tenant, TId, AbuseContext);
        {ok, normal} ->
            ok
    end,
    %% 2. Check aggregate abuse (flood)
    case router_payload_tracker:check_abuse_pattern(<<"_aggregate_">>, decide) of
        {abuse, targeted_tenant, AggregateContext} ->
            log_abuse_event(targeted_tenant, <<"_aggregate_">>, Subject, Payload, Headers, MsgId, AggregateContext),
            emit_abuse_metric(targeted_tenant, <<"_aggregate_">>, AggregateContext);
        _ ->
            ok
    end.

%% Internal: Validate and process message
-spec validate_and_process_message(binary(), binary(), map(), binary() | undefined, binary() | undefined) -> ok.
validate_and_process_message(Subject, Payload, Headers, MsgId, ReplyTo) ->
    case router_intake_validator:validate_intake_message(Subject, Payload, Headers, decide) of
        {ok, ValidatedMessage} ->
            process_validated_message(Subject, ValidatedMessage, Headers, MsgId, ReplyTo);
        {error, {ErrorCode, ErrorMessage, ErrorContext}} ->
            handle_validation_error(
                ErrorCode, ErrorMessage, ErrorContext, Subject, Payload, Headers, MsgId, ReplyTo
            )
    end.

%% Internal: Process validated message
-spec process_validated_message(binary(), map(), map(), binary() | undefined, binary() | undefined) -> ok.
process_validated_message(Subject, ValidatedMessage, Headers, MsgId, ReplyTo) ->
    ...
    case IdempotencyStatus of
        ...
        new ->
            handle_decide_request(Subject, ValidatedMessage, Headers, MsgId, ReplyTo)
    end.
...
-spec handle_validation_error(term(), binary(), map(), binary(), binary(), map(), binary() | undefined, binary() | undefined) -> ok.
handle_validation_error(ErrorCode0, ErrorMessage, ErrorContext, Subject, Payload, Headers, MsgId, ReplyTo) ->
    ErrorCode = normalize_error_code(ErrorCode0),
    ...
    %% Emit abuse metrics for specific error patterns
    case ErrorCode of
        payload_too_small ->
            TenantId = extract_tenant_id_from_payload(Payload),
            case TenantId of
                undefined -> ok;
                TId -> emit_abuse_metric(empty_payload, TId, FullContext)
            end;
        _ ->
            ok
    end,
...
-spec handle_decide_request(binary(), map(), map(), binary() | undefined, binary() | undefined) -> ok.
handle_decide_request(Subject, Request, Headers, MsgId, ReplyTo) ->
    ...
    case validate_tenant_if_enabled(
        TenantValidationEnabled, TenantId, ValidationContext, MsgId, Subject, Request, ReplyTo
    ) of
...
-spec validate_tenant_if_enabled(
    boolean(),
    binary() | undefined,
    map(),
    binary() | undefined,
...
validate_tenant_if_enabled(false, _TenantId, _ValidationContext, _MsgId, _Subject, _Request, _ReplyTo) ->
    ok;
validate_tenant_if_enabled(true, TenantId, ValidationContext, MsgId, Subject, Request, ReplyTo) ->
...
continue_decide_request(
    ...
    case CoreResult of
        {{ok, Decision}, _StopMetadata} ->
            ...
            ShouldPublish = should_publish_assignment(Request),
        ...
        {ok, Decision} ->
            ...
            %% Optional: push assignment to CAF if requested
            ShouldPublish2 = should_publish_assignment(Request),
            case ShouldPublish2 of
                true ->
                    spawn_monitor(fun() ->
                        router_caf_adapter:publish_assignment(Request, Decision)
                    end),
...
-spec process_message_with_backpressure_warning(binary(), binary(), map(), binary() | undefined, binary() | undefined) -> ok.
process_message_with_backpressure_warning(Subject, Payload, Headers, MsgId, ReplyTo) ->
...
extract_tenant_id_from_payload(Payload) ->
    try
        Decoded0 = jsx:decode(Payload, [{return_maps, true}]),
        TenantId0 = case Decoded0 of
            {incomplete, _} ->
                undefined;
            {with_tail, Map, _Tail} when is_map(Map) ->
                maps:get(~"tenant_id", Map, undefined);
            Map when is_map(Map) ->
                maps:get(~"tenant_id", Map, undefined);
            _ ->
                undefined
        end,
        normalize_tenant_id(TenantId0)
    catch
        _:_ ->
            undefined
    end.
...
log_abuse_event(heavy_payload, TenantId, Subject, _Payload, Headers, MsgId, AbuseContext) ->
...
log_abuse_event(targeted_tenant, TenantId, Subject, _Payload, Headers, MsgId, AbuseContext) ->
    RequestId = maps:get(~"request_id", AbuseContext, undefined),
    TraceId = maps:get(~"trace_id", Headers, maps:get(~"trace_id", AbuseContext, undefined)),
    
    router_logger:warn(~"Abuse detected: targeted tenant pattern", #{
...
-spec normalize_binary_config(term(), binary()) -> binary().
normalize_binary_config(Value, Default) ->
    case Value of
        Bin when is_binary(Bin) -> Bin;
        _ ->
            case list_to_binary_safe(Value) of
                undefined -> Default;
                Bin -> Bin
            end
    end.

-spec list_to_binary_safe(term()) -> binary() | undefined.
list_to_binary_safe(Value) ->
    case Value of
        Str when is_list(Str) ->
            %% Ensure valid bytes for iolist conversion to satisfy type checker and safety
            Bytes = [B || B <- Str, is_integer(B), B >= 0, B =< 255],
            case length(Bytes) =:= length(Str) of
                true -> list_to_binary(Bytes);
                false -> undefined
            end;
        _ ->
            undefined
...
-spec normalize_tenant_id(term()) -> binary() | undefined.
normalize_tenant_id(Value) ->
    case Value of
        Bin when is_binary(Bin) -> Bin;
        _ -> list_to_binary_safe(Value)
    end.

-spec normalize_error_code(term()) -> router_intake_error_codes:error_code().
normalize_error_code(Code) ->
    case Code of
        ...
        payload_too_small -> payload_too_small;
        internal_validation_error -> internal_validation_error;
        _ -> internal_validation_error
    end.
