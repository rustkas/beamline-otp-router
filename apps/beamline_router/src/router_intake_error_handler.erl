...
handle_intake_error(ErrorCode, ErrorMessage, Subject, Payload, _Headers, MsgId, Context) ->
    ...
    case should_send_error_response(Subject, Context) of
        true ->
...
build_and_send_error_response(Subject, ErrorCode, ErrorMessage, Context) ->
    ...
    ReplySubject = resolve_reply_subject(Subject, Context),
    router_nats:publish(ReplySubject, ErrorResponseJson),
    ok.

should_send_error_response(Subject, Context) ->
    is_request_reply_subject(Subject) orelse has_reply_to(Context).

has_reply_to(Context) ->
    case maps:get(~"reply_to", Context, undefined) of
        ReplyTo when is_binary(ReplyTo) -> true;
        _ -> false
    end.

resolve_reply_subject(_Subject, #{~"reply_to" := ReplyTo}) when is_binary(ReplyTo) ->
    ReplyTo;
resolve_reply_subject(Subject, _Context) ->
...
map_intake_error_to_gateway_code(ErrorCode) ->
    case ErrorCode of
        ...
        payload_too_small -> ~"invalid_request";
