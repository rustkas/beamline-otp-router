...
%% This test verifies that router_nats process can be started and attempt to connect.
%% It accepts both 'connected' and 'reconnecting' states since the connection
%% may take time to establish, especially in test environments without real NATS.
test_router_nats_connection(_Config) ->
    case router_nats_server:ensure_running() of
        {ok, _Pid} ->
            ...
            %% Accept connected or reconnecting (connection in progress)
            ?assert(lists:member(State, [connected, reconnecting, disconnected]),
                    {unexpected_state, State}),
            router_nats_server:stop_router_process(),
            router_nats_server:stop(),
            ok;
        {error, Reason} ->
            ct:log("NATS server not available: ~p - skipping test", [Reason]),
            ok
    end.
