...
test_route_telemetry_stop_event_no_exception(_Config) ->
    ...
    _ = router_ets_helpers:ensure_named_ets_table(Events, [named_table, set, public]),
    ...
    case router_ets_helpers:ets_lookup(Events, stop) of
        [] ->
    ...
    ok = test_helpers:wait_for_condition(
        fun() -> router_ets_helpers:ets_lookup(Events, stop) =/= [] end, 1000),
    [{stop, StopMeas, StopMeta}] = router_ets_helpers:ets_lookup(Events, stop),
    ...
    [] = router_ets_helpers:ets_lookup(Events, exception),
