...
test_randomized_failures(_Config) ->
    ...
    _ = router_ets_helpers:ensure_named_ets_table(FailureCounter, [set, public, {write_concurrency, true}, {read_concurrency, true}]),
    ...
    meck:expect(router_nats, publish, fun(_Subject, _Payload) ->
        ...
        case ShouldFail of
            true ->
                FailureCount = case router_ets_helpers:ets_lookup(FailureCounter, failures) of
                    [] -> 0;
                    [{failures, C}] -> C
                end,
    ...
    [{failures, TotalFailures}] = router_ets_helpers:ets_lookup(FailureCounter, failures),
...
test_during_message_processing(_Config) ->
    ...
    _ = router_ets_helpers:ensure_named_ets_table(ProcessedMessages, [set, public, {write_concurrency, true}, {read_concurrency, true}]),
    ...
    meck:expect(router_nats, publish, fun(_Subject, _Payload) ->
        ...
        case ShouldFail of
            true ->
                [{errors, E}] = router_ets_helpers:ets_lookup(ProcessedMessages, errors),
                ets:insert(ProcessedMessages, {errors, E + 1}),
                {error, timeout};
            false ->
                [{count, C}] = router_ets_helpers:ets_lookup(ProcessedMessages, count),
    ...
    [{count, SuccessCount}] = router_ets_helpers:ets_lookup(ProcessedMessages, count),
    [{errors, ErrorCount}] = router_ets_helpers:ets_lookup(ProcessedMessages, errors),
