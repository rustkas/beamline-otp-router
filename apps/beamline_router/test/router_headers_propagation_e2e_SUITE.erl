...
test_headers_propagation_rest_to_router(_Config) ->
    ...
    ExtractedHeaders = router_ets_helpers:ensure_named_ets_table(extracted_headers, [named_table, set, public]),
    ...
    try
        ...
        ?assertEqual([{trace_id, TraceId}], router_ets_helpers:ets_lookup(ExtractedHeaders, trace_id)),
        ?assertEqual([{span_id, SpanId}], router_ets_helpers:ets_lookup(ExtractedHeaders, span_id)),
        ?assertEqual([{tenant_id, TenantId}], router_ets_helpers:ets_lookup(ExtractedHeaders, tenant_id))
    after
...
test_headers_propagation_router_to_caf(_Config) ->
    ...
    PublishedHeaders = router_ets_helpers:ensure_named_ets_table(published_headers, [named_table, set, public]),
        ...
        case router_nats:publish_with_ack(Subject, Json, RequestMap) of
            {ok, _} ->
                ...
                test_helpers:wait_for_condition(fun() -> 
                    router_ets_helpers:ets_lookup(PublishedHeaders, trace_id) =/= []
                end, 1000),
                
                [{trace_id, PublishedTraceId}] = router_ets_helpers:ets_lookup(PublishedHeaders, trace_id),
                [{tenant_id, PublishedTenantId}] = router_ets_helpers:ets_lookup(PublishedHeaders, tenant_id),
...
test_headers_propagation_full_chain(_Config) ->
    ...
    RouterHeaders = router_ets_helpers:ensure_named_ets_table(router_headers, [named_table, set, public]),
    CAFHeaders = router_ets_helpers:ensure_named_ets_table(caf_headers, [named_table, set, public]),
    ...
    try
        ...
        ?assertEqual([{trace_id, TraceId}], router_ets_helpers:ets_lookup(RouterHeaders, trace_id)),
        ?assertEqual([{span_id, SpanId}], router_ets_helpers:ets_lookup(RouterHeaders, span_id)),
        ?assertEqual([{tenant_id, TenantId}], router_ets_helpers:ets_lookup(RouterHeaders, tenant_id)),
        ...
        case router_nats:publish_with_ack(CAFSubject, CAFJson, RequestMap) of
            {ok, _} ->
                ...
                test_helpers:wait_for_condition(fun() -> 
                    router_ets_helpers:ets_lookup(CAFHeaders, trace_id) =/= []
                end, 1000),
                
                [{trace_id, CAFTraceId}] = router_ets_helpers:ets_lookup(CAFHeaders, trace_id),
                [{tenant_id, CAFTenantId}] = router_ets_helpers:ets_lookup(CAFHeaders, tenant_id),
...
test_missing_headers_metric(_Config) ->
    ...
    MetricCalls = router_ets_helpers:ensure_named_ets_table(metric_calls, [named_table, set, public]),
    ...
    try
        ...
        test_helpers:wait_for_condition(fun() -> 
            router_ets_helpers:ets_lookup(MetricCalls, ctx_missing_headers_total) =/= []
        end, 1000),
        
        ?assertMatch([{ctx_missing_headers_total, _}], router_ets_helpers:ets_lookup(MetricCalls, ctx_missing_headers_total))
    after
        %% Always cleanup ETS table
