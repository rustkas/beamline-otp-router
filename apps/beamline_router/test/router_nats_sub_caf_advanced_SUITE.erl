...
test_custom_assignment_subject(_Config) ->
    ...
    SubjectCalls = router_ets_helpers:ensure_named_ets_table(subject_calls, [set, public]),
...
test_telemetry_metrics_incremented(_Config) ->
    ...
    TelemetryCalls = router_ets_helpers:ensure_named_ets_table(telemetry_calls, [set, public]),
...
test_async_retry_metrics(_Config) ->
    ...
    RetryCount = router_ets_helpers:ensure_named_ets_table(retry_count, [set, public]),
    ets:delete_all_objects(RetryCount),
    ets:insert(RetryCount, {count, 0}),
    
    meck:expect(router_nats, publish, fun(_Subject, _Payload) ->
        [{count, C}] = router_ets_helpers:ets_lookup(RetryCount, count),
