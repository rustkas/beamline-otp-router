...
test_nats_connection_loss_recovery(_Config) ->
    ...
    ConnState = router_ets_helpers:ensure_named_ets_table(connection_state, [named_table, set, public]),
    ...
    meck:expect(router_nats, publish, fun(_Subject, _Payload) ->
        [{connected, Connected}] = router_ets_helpers:ets_lookup(ConnState, connected),
        case Connected of
    ...
    meck:expect(router_nats, subscribe_jetstream, fun(_Subject, _Durable, _AckPolicy, _DeliverGroup, _Mode) ->
        [{connection_count, Count}] = router_ets_helpers:ets_lookup(ConnState, connection_count),
        NewCount = Count + 1,
    ...
    ?assert(is_process_alive(ConsumerPid)),
    [{connection_count, FinalCount}] = router_ets_helpers:ets_lookup(ConnState, connection_count),
    ?assert(FinalCount >= 1),
...
test_jetstream_consumer_reconnection(_Config) ->
    ...
    SubState = router_ets_helpers:ensure_named_ets_table(subscription_state, [named_table, set, public]),
    ...
    meck:expect(router_nats, subscribe_jetstream, fun(_Subject, _Durable, _AckPolicy, _DeliverGroup, _Mode) ->
        [{subscription_count, Count}] = router_ets_helpers:ets_lookup(SubState, subscription_count),
        NewCount = Count + 1,
    ...
    ?assert(is_process_alive(ConsumerPid)),
    [{subscription_count, FinalCount}] = router_ets_helpers:ets_lookup(SubState, subscription_count),
    ?assert(FinalCount >= 1),
...
test_ets_state_preservation_during_nats_restart(_Config) ->
    TestTable = router_ets_helpers:ensure_named_ets_table(router_jetstream_fi_test_ets, [named_table, public, set]),
    ...
    [{test_key, test_value}] = router_ets_helpers:ets_lookup(TestTable, test_key),
