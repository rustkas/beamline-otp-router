...
groups_for_level(heavy) -> [{group, gateway_integration_tests}].

groups() ->
    [
        {gateway_integration_tests, [sequence], [
            test_gateway_to_router_decide,
            test_gateway_to_router_error_handling,
            test_gateway_to_router_policy_not_found,
            test_gateway_to_router_rate_limiting,
...
test_gateway_to_router_overload_response(_Config) ->
    Subject = <<"beamline.router.v1.decide.overload">>,
    
    %% Setup: Create overload condition
    PendingTable = router_jetstream_pending_cache,
    true = ets:insert(PendingTable, {Subject, 1500, erlang:system_time(millisecond)}),
    
    LatencyTable = router_intake_latency_cache,
    true = ets:insert(LatencyTable, {{Subject, p95}, 6000, erlang:system_time(millisecond)}),
    ...
    %% Verify retry-after is present and correct in policy sub-map
    PolicyMap = maps:get(policy, Data, #{}),
    ?assert(maps:is_key(retry_after_seconds, PolicyMap)),
    ?assertEqual(30, maps:get(retry_after_seconds, PolicyMap)),
    
    %% Cleanup (delete keys, not tables)
    ets:delete(PendingTable, Subject),
    ets:delete(LatencyTable, {Subject, p95}),
    ok.
