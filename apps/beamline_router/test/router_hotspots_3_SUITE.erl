...
test_router_ets_guard_invariants(_Config) ->
    _ = router_ets_helpers:ensure_named_ets_table(test_table, [named_table, public, set, {read_concurrency, true}, {write_concurrency, true}]),
    Table = test_table,
    try
        Spec = #{
            type => set,
...
test_router_ets_guard_invalid_spec_keys(_Config) ->
    Table = test_table,
    _ = router_ets_helpers:ensure_named_ets_table(Table, [named_table, public, set]),
    try
        Spec = #{foo => bar},
...
test_router_ets_guard_detects_violations(_Config) ->
    Table = test_table,
    _ = router_ets_helpers:ensure_named_ets_table(
        Table,
        [named_table, public, bag, {read_concurrency, false}, {write_concurrency, false}, compressed]
    ),
    try
        Spec = #{
            type => set,
...
test_router_ets_guard_tid_tables(_Config) ->
    ...
    _ = router_ets_helpers:ensure_named_ets_table(
        Table,
        [bag, public, {read_concurrency, true}, {write_concurrency, true}]
    ),
    try
        Spec = #{
            type => set,
...
test_router_ets_guard_verify_table_violations(_Config) ->
    ...
    _ = router_ets_helpers:ensure_named_ets_table(
        Table,
        [named_table, public, bag, {keypos, 2}, {read_concurrency, false}, {write_concurrency, false}]
    ),
    try
        Spec = #{
            type => set,
...
test_router_ets_guard_telemetry_disabled_logging(_Config) ->
    Table = test_table,
    _ = router_ets_helpers:ensure_named_ets_table(Table, [named_table, public, bag, compressed]),
    try
        PrevTelemetry = application:get_env(beamline_router, telemetry_enabled, true),
...
test_router_ets_guard_boolean_violation_logging(_Config) ->
    ...
    _ = router_ets_helpers:ensure_named_ets_table(
        Table,
        [named_table, public, bag, {read_concurrency, true}, {write_concurrency, true}, compressed]
    ),
    try
        PrevTelemetry = application:get_env(beamline_router, telemetry_enabled, true),
        ok = application:set_env(beamline_router, telemetry_enabled, true),
