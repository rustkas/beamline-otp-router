...
benchmark_throughput_degradation_connect_fault(_Config) ->
    ...
    case BaselineTime > 0 of
        false ->
            ct:log("Baseline time too low for throughput calculation: ~p ms", [BaselineTime]),
            {skip, {baseline_time_too_low, BaselineTime}};
        true ->
            BaselineThroughput = (BaselineCount * 1000) / BaselineTime,
            case BaselineThroughput > 0 of
                false ->
                    ct:log("Baseline throughput too low: ~p ops/s", [BaselineThroughput]),
                    {skip, {baseline_throughput_too_low, BaselineThroughput}};
                true ->
                ...
                case FaultTime > 0 of
                    false ->
                        ct:log("Fault time too low for throughput calculation: ~p ms", [FaultTime]),
                        {skip, {fault_time_too_low, FaultTime}};
                    true ->
...
benchmark_latency_impact_connect_fault(_Config) ->
    ...
    case BaselineP95 > 0 of
        false ->
            ct:log("Baseline P95 too low for latency calculation: ~p us", [BaselineP95]),
            {skip, {baseline_p95_too_low, BaselineP95}};
        true ->
