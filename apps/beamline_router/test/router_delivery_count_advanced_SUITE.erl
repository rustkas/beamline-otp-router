...
groups_for_level(heavy) -> [{group, delivery_advanced_tests}];
groups_for_level(full) -> [{group, delivery_advanced_tests}];
groups_for_level(_) -> [].
...
groups() ->
    [{delivery_advanced_tests, [sequence], [
...
init_per_suite(Config) ->
    ...
    _ = router_ets_helpers:ensure_named_ets_table(router_delivery_count, [set, named_table, public, {write_concurrency, true}]),
...
test_cleanup_after_ack(_Config) ->
    ...
    Table = router_ets_helpers:ensure_named_ets_table(router_delivery_count, [set, named_table, public, {write_concurrency, true}]),
    
    ok = router_result_consumer:track_delivery_count(MsgId),
    [{MsgId, 1}] = router_ets_helpers:ets_lookup(Table, MsgId),
    
    ok = router_result_consumer:cleanup_delivery_count(MsgId),
    [] = router_ets_helpers:ets_lookup(Table, MsgId),
    ok.
...
test_concurrent_tracking(_Config) ->
    ...
    lists:foreach(fun(MsgId) ->
        case router_ets_helpers:ets_lookup(router_delivery_count, MsgId) of
...
test_under_ack_failures(_Config) ->
    ...
    [{MsgId, 3}] = router_ets_helpers:ets_lookup(router_delivery_count, MsgId),
    ok.
