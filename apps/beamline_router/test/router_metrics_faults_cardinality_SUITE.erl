...
test_cardinality_under_mass_load(_Config) ->
    ...
    lists:foreach(fun(N) ->
        ...
        router_metrics:emit_metric(router_test_metric, #{count => 1}, #{tenant_id => TenantId}),
        timer:sleep(10)
    end, lists:seq(1, 100)),
...
test_cardinality_with_many_tenants(_Config) ->
    ...
    lists:foreach(fun(N) ->
        ...
        router_metrics:emit_metric(router_tenant_metric, #{count => 1}, #{tenant_id => TenantId})
    end, lists:seq(1, 50)),
...
test_cardinality_limits_enforced(_Config) ->
    ...
    lists:foreach(fun(N) ->
        ...
        router_metrics:emit_metric(router_unique_metric, #{count => 1}, #{unique_id => UniqueId})
    end, lists:seq(1, 100)),
...
test_cardinality_with_high_cardinality_leak(_Config) ->
    ...
    lists:foreach(fun(N) ->
        ...
        router_metrics:emit_metric(router_leak_metric, #{count => 1}, #{request_id => UniqueId})
    end, lists:seq(1, 200)),
