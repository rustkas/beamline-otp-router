...
-define(DEGRADED_METRICS_TABLE, router_caf_degraded_metrics).
...
%% @doc Record degraded mode metric.
record_degraded(Metadata) ->
    case ensure() of
        ok ->
            true = ets:insert(?DEGRADED_METRICS_TABLE, {degraded, Metadata}),
            ok;
        Error ->
            Error
    end.

%% @doc Get degraded mode metric.
%% Returns:
%%   {ok, Metadata} | not_found | {error, Reason}
get_degraded() ->
    case ensure() of
        ok ->
            case ets:lookup(?DEGRADED_METRICS_TABLE, degraded) of
                [{degraded, Metadata}] ->
                    {ok, Metadata};
                [] ->
                    not_found
