...
test_queue_limit_adequacy(_Config) ->
    ...
    PrevMaxPending = application:get_env(beamline_router, nats_max_pending_operations, 1000),
    TestMaxPending = 50,
    ok = application:set_env(beamline_router, nats_max_pending_operations, TestMaxPending),
    
    %% Setup tracking
    meck:new(router_logger, [passthrough]),
    LogCalls = router_ets_helpers:ensure_named_ets_table(log_calls, [named_table, bag, public]),
    ...
    %% Seed pending queue to the limit to deterministically trigger the warning.
    SeedNow = erlang:system_time(millisecond),
    SeedOp = {publish, <<"test.subject">>, <<"payload_seed">>},
    SeedPending = lists:duplicate(TestMaxPending, {SeedOp, SeedNow}),
    _ = sys:replace_state(router_nats, fun(State0) ->
        State1 = setelement(2, State0, disconnected),
        State2 = setelement(3, State1, undefined),
        setelement(8, State2, SeedPending)
    end),
    ...
    ok = application:set_env(beamline_router, nats_max_pending_operations, PrevMaxPending),
...
test_retry_storm_prevention(_Config) ->
    ...
    MetricCalls = router_ets_helpers:ensure_named_ets_table(metric_calls, [named_table, bag, public]),
