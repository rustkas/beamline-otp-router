...
test_establish_baseline_metrics(Config) ->
    ...
    record_baseline_observations(Baseline),
    
    ct:print("[METRIC] Baseline Metrics Established:~n"
             "  [METRIC] sequential_throughput: ~p req/s~n"
             "  [METRIC] concurrent_throughput: ~p req/s~n"
             "  [METRIC] p95_latency_sequential: ~p ms~n"
             "  [METRIC] p95_latency_concurrent: ~p ms~n"
             "  [METRIC] memory_mb: ~p MB~n"
             "  [METRIC] ets_table_size: ~p entries~n",
             [
...
test_baseline_sequential_throughput(Config) ->
    ...
    %% Verify baseline meets minimum targets (lowered for initial baseline establishment)
...
record_baseline_observations(Baseline) ->
    SeqThroughput = maps:get(sequential_throughput, Baseline),
    ConThroughput = maps:get(concurrent_throughput, Baseline),
    SeqP95 = maps:get(p95_latency_sequential, Baseline),
    ConP95 = maps:get(p95_latency_concurrent, Baseline),
    SeqEntry = #{
        <<"suite">> => <<"router_performance_benchmark_SUITE">>,
        <<"level">> => <<"heavy">>,
        <<"workload">> => #{
            <<"scenario">> => <<"sequential">>,
        ...
        <<"metrics">> => #{
            <<"latency_ms">> => #{<<"p95">> => SeqP95},
            <<"throughput_rps">> => #{<<"avg">> => SeqThroughput},
    ...
    ConEntry = #{
        ...
        <<"metrics">> => #{
            <<"latency_ms">> => #{<<"p95">> => ConP95},
            <<"throughput_rps">> => #{<<"avg">> => ConThroughput},
            <<"error_rate">> => #{<<"fraction">> => 0.0}
        }
    },
    _ = record_perf_entry(SeqEntry),
    _ = record_perf_entry(ConEntry),
    ok.

record_perf_entry(Entry) ->
