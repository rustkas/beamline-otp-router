...
test_decide_concurrent_messages_no_global_blocking(_Config) ->
    ...
    PublishCalls = router_ets_helpers:ensure_named_ets_table(publish_calls, [named_table, set, public]),
    _ProcessedMessages = router_ets_helpers:ensure_named_ets_table(processed_messages, [named_table, set, public]),
    ...
    meck:expect(router_nats, publish, fun(_Subject, _Payload) ->
        Count = case router_ets_helpers:ets_lookup(PublishCalls, count) of
            [{count, C}] -> C + 1;
            [] -> 1
        end,
    ...
    [{count, FinalCount}] = router_ets_helpers:ets_lookup(PublishCalls, count),
...
test_decide_ets_cleanup_after_recovery(_Config) ->
    ...
    DeliveryTable = router_ets_helpers:ensure_named_ets_table(router_decide_delivery_counts, [named_table, public, set]),
    ...
    ok = test_helpers:wait_for_condition(
        fun() ->
            case router_ets_helpers:ets_lookup(DeliveryTable, MsgId) of
                [] -> true;
                [{MsgId, Count}] -> Count =< 5
            end
        end, 500),
    ...
    %% Verify cleanup
    case ets:info(DeliveryTable, name) of
        undefined -> ok;
        _ ->
            case router_ets_helpers:ets_lookup(DeliveryTable, MsgId) of
                [] -> ok;
